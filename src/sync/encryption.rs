...
//! encryption using AES-256-GCM. Session content is encrypted before being
//! written into the lore store ref and decrypted after being read back,
...
/// encryption and decryption across machines that share the salt and passphrase.
///
/// # Arguments
///
...
pub fn derive_key(passphrase: &str, salt: &[u8]) -> Result<Vec<u8>, SyncError> {
    ...
    let hash_output = hash
        .hash
        .ok_or_else(|| SyncError::Encryption("No hash output".to_string()))?;

    // Take the first KEY_SIZE bytes as the encryption key.
    let key_bytes = hash_output.as_bytes();
    if key_bytes.len() < KEY_SIZE {
        return Err(SyncError::Encryption("Derived key too short".to_string()));
    }
...
///
/// The salt is stored in the ref tree at `meta/salt` (it is not secret) so
/// other machines derive the same key from the same passphrase.
...
/// # Returns
///
/// The encrypted data with prepended nonce, suitable for writing as a git blob.
pub fn encrypt_data(data: &[u8], key: &[u8]) -> Result<Vec<u8>, SyncError> {
    if key.len() != KEY_SIZE {
    ...
    let cipher = Aes256Gcm::new_from_slice(key)
        .map_err(|e| SyncError::Encryption(format!("Cipher creation failed: {e}")))?;
...
pub fn decrypt_data(data: &[u8], key: &[u8]) -> Result<Vec<u8>, SyncError> {
    if key.len() != KEY_SIZE {
        return Err(SyncError::Encryption(format!(
            "Invalid key size: expected {KEY_SIZE}, got {}",
            key.len()
...
pub fn decode_key_hex(hex_str: &str) -> Result<Vec<u8>, SyncError> {
    hex::decode(hex_str).map_err(|e| SyncError::Encryption(format!("Hex decode failed: {e}")))
}

// Minimal hex encoding helper to avoid an extra dependency.
mod hex {
