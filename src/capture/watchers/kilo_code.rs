...
//!
//! Parses conversation history from Kilo Code, an AI coding assistant VS Code
//! extension that is a fork of Roo Code.
//!
...
///
/// Discovers and parses task conversation files from Kilo Code's VS Code
/// extension storage.
pub struct KiloCodeWatcher;

impl Watcher for KiloCodeWatcher {
    fn info(&self) -> WatcherInfo {
        WatcherInfo {
            name: "kilo-code",
            description: "Kilo Code VS Code extension sessions",
            default_paths: vec![kilo_code_tasks_path()],
        }
    }

    fn is_available(&self) -> bool {
        kilo_code_tasks_path().exists()
    }

    fn find_sources(&self) -> Result<Vec<PathBuf>> {
        find_kilo_code_tasks()
    }

    fn parse_source(&self, path: &Path) -> Result<Vec<(Session, Vec<Message>)>> {
        let parsed = parse_kilo_code_task(path)?;
        match parsed {
    ...
    fn watch_paths(&self) -> Vec<PathBuf> {
        vec![kilo_code_tasks_path()]
    }
}

/// Returns the path to Kilo Code's tasks storage directory.
///
/// This is platform-specific:
/// - macOS: `~/Library/Application Support/Code/User/globalStorage/kilocode.Kilo-Code/tasks`
/// - Linux: `~/.config/Code/User/globalStorage/kilocode.Kilo-Code/tasks`
/// - Windows: `%APPDATA%/Code/User/globalStorage/kilocode.Kilo-Code/tasks`
fn kilo_code_tasks_path() -> PathBuf {
    let base = get_vscode_global_storage();
    base.join("kilocode.Kilo-Code").join("tasks")
}
...
/// Finds all Kilo Code task directories.
///
/// Each task has its own subdirectory containing conversation files.
fn find_kilo_code_tasks() -> Result<Vec<PathBuf>> {
    let tasks_path = kilo_code_tasks_path();
...
/// Raw Kilo Code API conversation message.
/// Uses the same format as Roo Code and Cline.
#[derive(Debug, Deserialize)]
struct KiloCodeApiMessage {
    /// Role: "user" or "assistant"
    role: String,

    /// Message content (can be string or array of content blocks)
    content: KiloCodeContent,
...
/// Content in Kilo Code API format.
/// Uses the same format as Roo Code and Cline.
#[derive(Debug, Deserialize)]
#[serde(untagged)]
enum KiloCodeContent {
    /// Simple text content
    Text(String),
    /// Array of content blocks
    Blocks(Vec<KiloCodeContentBlock>),
}

impl KiloCodeContent {
        ...
        match self {
            ...
            Self::Blocks(blocks) => blocks
                ...
                .filter_map(|b| match b {
                    KiloCodeContentBlock::Text { text } => Some(text.clone()),
                    _ => None,
                })
                .collect::<Vec<_>>()
                .join("\n"),
...
/// A content block in Kilo Code messages.
/// Uses the same format as Roo Code and Cline.
#[derive(Debug, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
enum KiloCodeContentBlock {
    /// Text content
    Text { text: String },
    /// Image content
    Image {
...
/// Task metadata from Kilo Code.
/// Uses the same format as Roo Code and Cline.
#[derive(Debug, Deserialize, Default)]
struct KiloCodeTaskMetadata {
...
/// Parses a Kilo Code task from its conversation history file.
fn parse_kilo_code_task(history_path: &Path) -> Result<Option<(Session, Vec<Message>)>> {
    let content = fs::read_to_string(history_path)
        .context("Failed to read Kilo Code conversation history")?;

    let raw_messages: Vec<KiloCodeApiMessage> =
        serde_json::from_str(&content).context("Failed to parse Kilo Code conversation JSON")?;
    ...
    let metadata = task_dir
        ...
        .and_then(|c| serde_json::from_str::<KiloCodeTaskMetadata>(&c).ok())
        .unwrap_or_default();
...
mod tests {
    ...
    /// Creates a temporary Kilo Code conversation file with given JSON content.
    ...
    fn test_watcher_info() {
        let watcher = KiloCodeWatcher;
        let info = watcher.info();

        assert_eq!(info.name, "kilo-code");
        assert_eq!(info.description, "Kilo Code VS Code extension sessions");
    }
    ...
    fn test_watcher_watch_paths() {
        ...
        assert!(paths[0].to_string_lossy().contains("kilocode.Kilo-Code"));
    ...
    fn test_parse_simple_conversation() {
        ...
        let result = parse_kilo_code_task(file.path()).expect("Should parse");

        let (session, messages) = result.expect("Should have session");
        assert_eq!(session.tool, "kilo-code");
    ...
    fn test_find_tasks_returns_ok_when_dir_missing() {
        let result = find_kilo_code_tasks();
        assert!(result.is_ok());
    }
    ...
    fn test_parse_with_task_directory() {
        ...
        let result = parse_kilo_code_task(&history_path).expect("Should parse");
